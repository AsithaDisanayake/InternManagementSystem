...
@section('content')
    ...
    <div class="container">
        <div class="card card-register mx-auto mt-5">
            ...
            <div class="card-body">
                <form action="{{url('/setStudentPreferences')}}" method="post" enctype="multipart/form-data">
                    {{csrf_field()}}
...
{{--@section('script')--}}
    {{--<script type="text/javascript">--}}
        ...
        {{--function getAll() {--}}
            {{--ajax({--}}
                {{--url:'{{url('viewStudentPreferencesForm')}}',--}}
                {{--type:'POST',--}}
                {{--dataType: 'JSON',--}}
                {{--data: {registration_no: '{{Auth::user()->username}}'}--}}
            {{--})--}}
                {{--.done(function () {--}}
                    {{--console.log('success');--}}
                {{--})--}}
